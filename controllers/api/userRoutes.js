...
//will reroute to /
router.post('/login', async (req, res) => {  //http://localhost:3001/api/users/login
...
router.delete('/logout/:id', async (req, res) => { //http://localhost:3001/api/users/logout/1
    try {
        const targetUserId = parseInt(req.params.id, 10);

        if (req.session.user_id !== targetUserId) {
            res.status(403).json({ message: 'You can only log out of your own session' });
            return;
        }
...
router.get('/debug', async (req, res) => { //http://localhost:3001/api/users/debug
    try {
...
router.get('/debug/open-sessions', async (req, res) => { //http://localhost:3001/api/users/debug/open-sessions
    try {
...
router.get('/debug/logout-all', async (req, res) => { //http://localhost:3001/api/users/debug/logout-all
    try {
